...
public class Test_ref1_kinectic{
	...
	public static void main(String[] args){
		...
		ref1_kinectic machine = new ref1_kinectic();
		int n = 23; //the number of events in the machine
		...
		while (true){
			...
			case 5: if (machine.evt_ADD_OBJECT.guard_ADD_OBJECT(Desc,H,Obj,W,X,Y))
				machine.evt_ADD_OBJECT.run_ADD_OBJECT(Desc,H,Obj,W,X,Y); break;
			case 6: if (machine.evt_ADD_LANE.guard_ADD_LANE(Finish,Lane,Left,Right,F))
				machine.evt_ADD_LANE.run_ADD_LANE(Finish,Lane,Left,Right,F); break;
			case 7: if (machine.evt_FINISHED_LANE.guard_FINISHED_LANE(Car,Lane))
				machine.evt_FINISHED_LANE.run_FINISHED_LANE(Car,Lane); break;
			case 8: if (machine.evt_ADD_CAR.guard_ADD_CAR(Car,Desc,H,W,X,Y,F,M))
				machine.evt_ADD_CAR.run_ADD_CAR(Car,Desc,H,W,X,Y,F,M); break;
			case 9: if (machine.evt_DELETE_CAR.guard_DELETE_CAR(Car))
				machine.evt_DELETE_CAR.run_DELETE_CAR(Car); break;
			case 10: if (machine.evt_SET_DRIFT.guard_SET_DRIFT(Car,D))
				machine.evt_SET_DRIFT.run_SET_DRIFT(Car,D); break;
			case 11: if (machine.evt_ADD_OBSTACLE.guard_ADD_OBSTACLE(Desc,H,Obs,W,X,Y))
				machine.evt_ADD_OBSTACLE.run_ADD_OBSTACLE(Desc,H,Obs,W,X,Y); break;
			case 12: if (machine.evt_DELETE_OBSTACLE.guard_DELETE_OBSTACLE(Obs))
				machine.evt_DELETE_OBSTACLE.run_DELETE_OBSTACLE(Obs); break;
			case 13: if (machine.evt_SET_VEL.guard_SET_VEL(Car,V))
				machine.evt_SET_VEL.run_SET_VEL(Car,V); break;
			case 14: if (machine.evt_SET_ACC.guard_SET_ACC(A,Car))
				machine.evt_SET_ACC.run_SET_ACC(A,Car); break;
			case 15: if (machine.evt_SET_MAXVEL.guard_SET_MAXVEL(Car,M))
				machine.evt_SET_MAXVEL.run_SET_MAXVEL(Car,M); break;
			case 16: if (machine.evt_UPDATE_VEL.guard_UPDATE_VEL(Car,Elapsed))
				machine.evt_UPDATE_VEL.run_UPDATE_VEL(Car,Elapsed); break;
			case 17: if (machine.evt_SET_ZERO_VEL.guard_SET_ZERO_VEL(Car))
				machine.evt_SET_ZERO_VEL.run_SET_ZERO_VEL(Car); break;
			case 18: if (machine.evt_APPLY_FRICTION.guard_APPLY_FRICTION(Elapsed,Lane,Obj))
				machine.evt_APPLY_FRICTION.run_APPLY_FRICTION(Elapsed,Lane,Obj); break;
			case 19: if (machine.evt_CAR_RESET.guard_CAR_RESET(Car,Lane))
				machine.evt_CAR_RESET.run_CAR_RESET(Car,Lane); break;
			case 20: if (machine.evt_WALL_COLLISION.guard_WALL_COLLISION(Car,Lane))
				machine.evt_WALL_COLLISION.run_WALL_COLLISION(Car,Lane); break;
			case 21: if (machine.evt_OBJECT_COLLISION.guard_OBJECT_COLLISION(MyCar,Obj))
				machine.evt_OBJECT_COLLISION.run_OBJECT_COLLISION(MyCar,Obj); break;
			case 22: if (machine.evt_UPDATE_POS.guard_UPDATE_POS(Elapsed,Obj))
