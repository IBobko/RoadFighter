...
public class Test_ref2_scoring{
	...
	public static void main(String[] args){
		...
		ref2_scoring machine = new ref2_scoring();
		...
		while (true){
			...
			case 7: if (machine.evt_SET_DRIFT.guard_SET_DRIFT(Car,D))
			...
			case 10: if (machine.evt_ADD_OBSTACLE.guard_ADD_OBSTACLE(Desc,H,Obs,W,X,Y))
				machine.evt_ADD_OBSTACLE.run_ADD_OBSTACLE(Desc,H,Obs,W,X,Y); break;
			case 11: if (machine.evt_DELETE_OBSTACLE.guard_DELETE_OBSTACLE(Obs))
				machine.evt_DELETE_OBSTACLE.run_DELETE_OBSTACLE(Obs); break;
			case 12: if (machine.evt_SET_VEL.guard_SET_VEL(Car,V))
				machine.evt_SET_VEL.run_SET_VEL(Car,V); break;
			case 13: if (machine.evt_SET_ACC.guard_SET_ACC(A,Car))
				machine.evt_SET_ACC.run_SET_ACC(A,Car); break;
			case 14: if (machine.evt_SET_MAXVEL.guard_SET_MAXVEL(Car,M))
				machine.evt_SET_MAXVEL.run_SET_MAXVEL(Car,M); break;
			case 15: if (machine.evt_UPDATE_VEL.guard_UPDATE_VEL(Car,Elapsed))
				machine.evt_UPDATE_VEL.run_UPDATE_VEL(Car,Elapsed); break;
			case 16: if (machine.evt_APPLY_FRICTION.guard_APPLY_FRICTION(Elapsed,Lane,Obj))
				machine.evt_APPLY_FRICTION.run_APPLY_FRICTION(Elapsed,Lane,Obj); break;
			case 17: if (machine.evt_CAR_RESET.guard_CAR_RESET(Car,Lane))
				machine.evt_CAR_RESET.run_CAR_RESET(Car,Lane); break;
			case 18: if (machine.evt_WALL_COLLISION.guard_WALL_COLLISION(Car,Lane))
				machine.evt_WALL_COLLISION.run_WALL_COLLISION(Car,Lane); break;
			case 19: if (machine.evt_OBJECT_COLLISION.guard_OBJECT_COLLISION(MyCar,Obj))
				machine.evt_OBJECT_COLLISION.run_OBJECT_COLLISION(MyCar,Obj); break;
			case 20: if (machine.evt_FINISHED_LANE.guard_FINISHED_LANE(Car,Lane))
				machine.evt_FINISHED_LANE.run_FINISHED_LANE(Car,Lane); break;
			case 21: if (machine.evt_UPDATE_POS.guard_UPDATE_POS(Elapsed,Obj))
				machine.evt_UPDATE_POS.run_UPDATE_POS(Elapsed,Obj); break;
			case 22: if (machine.evt_UPDATE_SCORE.guard_UPDATE_SCORE(Car,S))
